...
class Offer(API):
    ...
    def get(self, offer_id=None):
        ...
        """
        return super(Offer, self).prepare_request('GET', offer_id)

    def download_landing(self, offer_id):
        """
        Downloads local landing
        """
        return super(Offer, self).prepare_request('GET', offer_id, 'download')

    def files_structure(self, offer_id):
        """
        Returns files structure of local landing
        """
        return super(Offer, self).prepare_request(
            'GET', offer_id, 'get_structure')
    ...
    def add_file(self, offer_id, file_path):
        ...
        """
        return super(Offer, self).prepare_request(
            'POST', offer_id, 'add_file', path=file_path)

    def clone(self, offer_id):
        """
        Clones offer by its id
        """
        return super(Offer, self).prepare_request('POST', offer_id, 'clone')
